...
TIME_SIMU = 30.0       # durée de la simulation (minutes)
...
D_CAMP = 15.0  # Diffusion du cAMP (µm²/min)
D_PDE = 10.0  # Diffusion de la PDE (µm²/min)
rho = 1  # Production basale de cAMP
alpha0 = 3e-8  # Facteur normalisation pour la production cAMP
J = 0.08  # Taux de dégradation global du cAMP (min^-1)
k_PDE = 1000.0  # Taux dégradation cAMP par PDE (min^-1)
PDE_inhibition_threshold = 1e-4  # (5e-4?) Seuil PDE pour inhibition progressive cAMP

# ------------------
# Martiel–Goldbeter (paramètres cellulaires)
# ------------------
F1_base = 0.1     # taux de désensibilisation de base dont dépend r_T la fraction de récepteurs actifs
F2_base = 0.1     # taux de réactivation de base dont dépend r_T la fraction de récepteurs actifs
n = 3 # 4.0           # exposant de Hill (récepteur)
K_h = 0.5 #0.8         # constante demi-sat. pour désensibilisation pour la chimie interne

q_s = 1e-3  # (1e-3) production intracellulaire (prod cAMP par min)
...
hill_K_h = 1e-6 # 3.3e-7    # constante demi-sat. pour boucle de rétroaction
cAMP_max = 1e-4  # Le feedback maximal est atteint pour une concentration de 1e-6

# ------------------
# Paramètres pour la production de PDE
# ------------------
hill_n_PDE = 2 # exposant de Hill pour la production de PDE
PDE_rate = 1e-8 # 1e-4  # Production de PDE (unités arbitraires)
PDE_threshold = 4e-3 #4e-5  # Seuil de production de PDE
PDE_decay = 0.3 #0.1  # Dégradation basique PDE (min^-1)
ramp_up   = 1.2  # Vitesse d'augmentation de production de PDE
ramp_down = 1.8  # Vitesse de diminution de production de PDE
k_PDE = 1  # Taux dégradation cAMP par PDE (min^-1)
...
ALPHA_CHEMO = 500.0    # Paramètre de saturation (à ajuster selon l'échelle du cAMP)
...
velocity_magnitude_pop1 = 0.1         # Vitesse moyenne (μm/min) pour Pop 1
...
def plot_cells_and_fields(cells, camp_grid, pde_grid, iteration, time_now,
    ...
    cAMP_img = ax1.imshow(
        ...
        cmap='viridis', norm=LogNorm(vmin=1e-9,vmax=1e-3) 
    )
