...
Le script enregistre également le tracé des oscillations (A, R et cAMP local) pour la première cellule.
"""
...
class CellAgent:
    def __init__(self, id, pop, position, velocity, velocity_magnitude, persistence,
        ...
        Paramètres:
            ...
            A, R                       : États internes du modèle FHN.
    ...
    def update_state(self, signal_value, dt):
        ...
        self.A += dA
        # Ici, le terme constant est remplacé par la concentration locale en cAMP
...
class Population:
    def __init__(self, num_cells, space_size, velocity_magnitude, persistence, min_distance,
        ...
        Paramètres:
            ...
            A_init, R_init           : États initiaux pour les cellules.
        """
                ...
                while not placed:
                    ...
                    if not conflicts:
                        ...
                        new_cell = CellAgent(cell_id_counter, self.pop_tag, position, velocity,
                                              ...
                                              self.sensitivity_cAMP_threshold, basal_value, self.A_init, self.R_init)
...
class cAMP:
    ...
    def compute_laplacian_9point(self, S):
        ...
        # 3) Combinaison
        # Il existe plusieurs formulations possibles. 
        # Par exemple, un stencil "9 points" isotrope courant est :
        # Lap(S) = [ -20*S 
        #            + 4*(S_up + S_down + S_left + S_right)
        #            + 2*(S_upleft + S_upright + S_downleft + S_downright) ] / (6 * dx^2)
    ...
    def update(self, cells):
        ...
        Met à jour le champ de cAMP en intégrant :
            - La production locale de cAMP par les cellules (déclenchée uniquement si le cAMP local > 0).
        ...
        if cells:
            for cell in cells:
                ...
                # On déclenche la production uniquement si le cAMP local est supérieur à zéro
                if local_signal > 0:
...
SPACE_SIZE = 100  # μm  # Taille de l'espace de simulation (longueur d'un côté du carré)
...
# =============================================================================
cell_params = {
    'c0': 0.2, # 0.5,         # a.u. - Terme constant influençant l'évolution de R (stabilise les oscillations)
    'a': 0.1, #2.0,          # a.u. - Intensité du terme de stimulation dans l'équation de A (impacte l'excitabilité)
    'gamma': 0.1,#5, #2,        # min⁻¹ - Facteur de couplage entre A et R (contrôle la relaxation de R)
    ...
    'D': 50.0,     # a.u. - Quantité de cAMP produite par une cellule lorsque A dépasse le seuil af
    'a0': 1.0, #150,          # a.u. - Production basale de cAMP, à utiliser pour certaines cellules
    ...
    'D_cAMP': 0.2, # 150.0,   # μm²/min - Coefficient de diffusion du cAMP
    'aPDE': 0.5, # 150,        # min⁻¹ - Taux de dégradation du cAMP
    'grid_resolution': 0.1, # 0.5,  # μm - Taille d'une case de la grille
...
FLUCTUATION_FACTOR = 0#3    # Facteur de fluctuation aléatoire
...
population1 = Population(num_cells=pop1, space_size=SPACE_SIZE,
                         ...
                         basal_fraction=0.001, A_init = 0, R_init = -1)
...
if INITIAL_AMPc:
    # Injection initiale de cAMP aux positions des cellules ayant production basale
    for cell in cells:
...
while time < TIME_SIMU:
    ...
    if INCLUDE_CELLS:
        ...
        # displacement = torch.tensor(0.0, device=device)
...
# =============================================================================
# Tracé des oscillations pour la première cellule (cellule 0)
# =============================================================================
...
ax2.set_ylabel("Concentration locale de cAMP")
ax.legend(loc='upper left')
ax2.legend(loc='upper right')
plt.title("Oscillations d'une cellule FHN couplée au champ de cAMP")
plt.tight_layout()
